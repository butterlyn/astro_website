...
| Zero Trust organization and Free plan      | Not yet verified. The [account audit](https://github.com/butterlyn/astro_website/actions/runs/35060031195) receives HTTP 403 for the organization. That response does not establish whether an organization exists.                                                                                 |
| Editing Access application and email login | You confirm an application for `edit.leer.education` exists in the dashboard. Its AUD, coverage and policy still need verification. The API audit did not find it or a One-time PIN provider; the updated audit checks modern public-hostname fields and pagination to investigate the discrepancy. |
...
In the main Cloudflare dashboard, open **My Profile → API Tokens** and edit the existing token used by GitHub's `CLOUDFLARE_API_TOKEN` secret. Add these account permissions:
