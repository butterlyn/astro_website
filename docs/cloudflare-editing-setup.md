...
Complete Cloudflare Access first; the maintainer then deploys the editor; finally you log in through the website to finish Tina's onboarding. Creating a Tina project or an Access application does not deploy the website. The AUD tag is a **Cloudflare Access application identifier** and is available before the website works. **The editor was deployed on 16 September: the next user step is [the first hosted login](#7-complete-the-first-hosted-login).**
...
| Zero Trust organization and Free plan      | The [updated account audit](https://github.com/butterlyn/astro_website/actions/runs/35062674819) verifies `tight-snowflake-ad6f.cloudflareaccess.com` in the deployment account. The team domain also publishes signing keys. Free-plan entitlement and hosted usage remain acceptance checks.                                                                                                                |
| Editing Access application and email login | Verified by the updated audit: one application covering the entire editing hostname, matching AUD, six-hour sessions, One-time PIN only, and one Allow policy for `admin@leer.education`. WARP authentication and preflight bypass are disabled.                                                                                                                                                              |
| Hosted editing website                     | Deployed successfully at [edit.leer.education/edit/](https://edit.leer.education/edit/) by the [protected preview workflow](https://github.com/butterlyn/astro_website/actions/runs/35063110881). The [post-deployment audit](https://github.com/butterlyn/astro_website/actions/runs/35063425168) confirms that only this hostname is attached to `leer-editing`; the public site remains on `leer-preview`. |
| Tina “Log in through your site”            | Deployment is complete and you confirmed that desktop access now works after the initial DNS failure. The router also resolves the hostname correctly. You then confirmed reaching the editing reservation page after Cloudflare sign-in. Tina sign-in, loading the proof document and a real hosted save are the next checkpoints.                                                                           |
...
The GitHub-held Cloudflare token must read the organization, application, policy and login provider before deployment. After you saved the two permissions below, the [audit succeeded for these resources](https://github.com/butterlyn/astro_website/actions/runs/35062674819). This confirms that you edited the token used by GitHub; the suspected token mismatch is resolved. Its original use for the website before Tina is expected: this Cloudflare credential deploys the Worker, while `TINA_TOKEN` reads Tina content.
...
## A DNS error after first deployment

The first deployment creates the hostname's DNS record. A browser or network that looked it up earlier can retain a negative answer. On 16 September, Cloudflare, Google and Quad9 resolved the new hostname and HTTPS requests reached the Access login page, while the local router still returned “name does not exist” for its IPv4 record.

The maintainer successfully ran `ipconfig /flushdns` on the shared Windows host; the router's negative answer remained. This command clears Windows' DNS cache, including negative entries, but does not clear a router's upstream cache. [Windows DNS cache command](https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/ipconfig).

To continue while that answer expires, Chrome can use **Settings → Privacy and security → Security → Use secure DNS → Cloudflare (1.1.1.1)**. Reload the editing URL afterward. This changes Chrome's DNS provider; the previous browser setting can be restored after normal resolution works. Other browser paths are in [Cloudflare's browser DNS instructions](https://developers.cloudflare.com/1.1.1.1/encryption/dns-over-https/encrypted-dns-browsers/). If it still fails, report the exact error before changing any hosting or Access configuration.

The originator subsequently reached the site over mobile data and then confirmed that desktop access worked. A fresh router lookup also returned the correct IPv4 addresses. The DNS issue is resolved. The originator then reached the editing reservation page after Cloudflare sign-in; Tina sign-in and saving remain separate checkpoints.
