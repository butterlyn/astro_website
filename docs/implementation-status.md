...
- Bootstrap source `9d2ca8d` is published on `development`, now GitHub's default branch. [Setup validation passed](https://github.com/butterlyn/astro_website/actions/runs/34876050675). The first Website validation run exposed Wrangler's missing output-parent directory on a fresh checkout; `types:worker` now creates that directory explicitly. The corrected run is pending.
